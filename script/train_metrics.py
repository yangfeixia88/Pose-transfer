...
class FID():
    ...
    def calculate_from_disk(self, generated_path, gt_path):
        ...
        return fid_value
    def calculate_image(self,gt_images,generated_images):
        print('calculate gt_images statistics...')
        m1, s1 = self.calculate_activation_statistics(gt_images, self.verbose)
        print('calculate generated_images statistics...')
        m2, s2 = self.calculate_activation_statistics(generated_images, self.verbose)
        fid_value = self.calculate_frechet_distance(m1, s1, m2, s2)
        print('fid_distance %f' % (fid_value))
        return fid_value
    ...
    def get_activations(self, images, verbose=False):
        ...
        pred_arr = np.empty((n_used_imgs, self.dims))  #(256,2048)
        for i in range(n_batches):
            ...
            batch = torch.from_numpy(images[start:end]).type(torch.FloatTensor)   #[1,3,256,256]
...
class Reconstruction_Metrics():
    ...
    def calculate_from_disk(self, inputs, gts, save_path=None, sort=True, debug=0):
        ...
        return dic
    def calculate_psnr_image(self,image1,image2):

        psnr = compare_psnr(image1, image2, data_range=self.data_range)
        # ssim = compare_ssim(image1, image2, data_range=self.data_range,
        #                          win_size=self.win_size, multichannel=self.multichannel)
        # ssim =0
        print(
            "PSNR: %.4f" % round(np.mean(psnr), 4),
...
class LPIPS():
    ...
    def __call__(self, image_1, image_2):
        ...
        """
        image_1 = torch.zeros(1, 3, 64, 64)
        image_2 = torch.zeros(1, 3, 64, 64)
    ...
    def calculate_from_disk(self, path_1, path_2, batch_size=64, verbose=False, sort=True):
        ...
        return distance
    def calculate_lpips_image(self,imgs_1,imgs_2,batch_size=64, verbose=False):
        # if sort:
        #     files_1 = sorted(get_image_list(path_1))
        #     files_2 = sorted(get_image_list(path_2))
        # else:
    ...
    def calculate_mask_lpips(self, path_1, path_2, batch_size=64, verbose=False, sort=True):
        ...
        for i in range(len(files_1)):
            ...
            mask = np.tile(np.expand_dims(create_masked_image(string).astype(np.float32), -1),
                           (1, 1, 3))  # .repeat(1,1,3)
...
if __name__ == "__main__":
    ...
    image1_path ='../demo_results/image/truth_image.jpg'
    image2_path ='../demo_results/image/generated_image.jpg'
    # image_path = '../demo_results/fashion/整体/715/500000/'
    # image_name1 = 'fashionMENShirts_Polosid0000113801_2side_2_fashionMENShirts_Polosid0000113801_1front_vis'
    # image_name2 = 'fashionMENShirts_Polosid0000113801_2side_2_fashionMENShirts_Polosid0000113801_1front_gt'
    ...
    # image2 /= 255
    image1 = np.array([imread(str(image1_path)).astype(np.float32)])
    # print(imgs)
    # Bring images to shape (B, 3, H, W)
    # image1 = image1.transpose((0, 3, 1, 2))
    image1 /= 255.0
    # print(imgs.shape)
    image2 = np.array([imread(str(image2_path)).astype(np.float32)])
    # print(imgs)
    # Bring images to shape (B, 3, H, W)
    ...
    rec_dic_1 = rec.calculate_psnr_image(image1, image2)
    print('calculate LPIPS...')
    imgs_1 = np.array([imread(str(image1_path)).astype(np.float32) / 127.5 - 1 ])
    imgs_2 = np.array([imread(str(image2_path)).astype(np.float32) / 127.5 - 1 ])
    # lpips_score = lpips.calculate_from_disk(distorated_list, gt_list, sort=False)
    lpips_score = lpips.calculate_lpips_image(imgs_1, imgs_2)
