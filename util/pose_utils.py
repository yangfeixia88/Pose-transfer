...
if __name__ == "__main__":
    ...
    import os
    i = 5
    df = pd.read_csv('../dataset/market/market-annotation-test.csv', sep=':')
    # print(df)
    for index, row in df.iterrows():
        ...
        print("mmm的大小",mmm.shape)
        img = imread('../dataset/market/test/' + row['name'])
        # print("图片的大小",img.shape)
        # plt.imshow(img)
        # plt.show()
        ...
        plt.imshow(mmm)
        # 将图片存储在本地文件夹中
        plt.savefig('../dataset/market/test_points/'+ row['name'])
        # plt.show()
