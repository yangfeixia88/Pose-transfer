...
class BaseModel():
    def __init__(self, opt):
        ...
        self.value_names = []
        self.image_paths = []
        self.Edge_paths = os.path.join(opt.checkpoints_dir, opt.edge_name)
        self.optimizers = []
        self.schedulers = []
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                try:
                    ...
                    if name == 'G' and self.model == 'pose' :
                        edge_path = os.path.join(self.Edge_paths, filename)
                        pretrained_dict_edge = torch.load(edge_path)
                    # model_dict_ = model_dict
                    # from copy import deepcopy
                    # count = 0
                    # for key,nkey in zip(pretrained_dict.keys(),model_dict.keys()):
                    #     if key != nkey:
                    #         count +=1
                    #         print("不相等")
                    #         print(key)
                    #         print(nkey)
                    #         model_dict[nkey] = deepcopy(pretrained_dict[key])
                    #     else:
                    #         model_dict[key] = deepcopy(pretrained_dict[key])
                    # print(count)
                    try:
                        from copy import deepcopy
                        model_dict_ = pretrained_dict.copy()
                        model_dict_.update(pretrained_dict_edge)

                        pretrained_dict_ = {k: v for k, v in model_dict_.items() if k in model_dict}
                        # for k,v in pretrained_dict.items():
                        #     if k in model_dict_:
                        #         model_dict_[k]=v
                        # if name == 'G' and self.model == 'pose':
                        #     for key,value in pretrained_dict_edge.items():
                        #         if key in model_dict_:
                        #             model_dict_[key] = value
                                # print(torch.eq(value,model_dict[s+key]))
                            # net.load_state_dict(model_dict)
                        # model_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
                        # expr_dir = os.path.join(self.opt.checkpoints_dir,self.opt.name)
                        # file = os.path.join(expr_dir,'train.txt')
                        # with open(file,'wt') as file_meg:
                        #     for k,v in pretrained_dict:
                        #         file_meg.write('%s: %s\n' % (str(k), str(v)))
                        # print(pretrained_dict_)
                        # s = "edge_net.model_edge."
                        # pretrained_dict_edge_ = {s+k:v for k,v in pretrained_dict.items() if s+k in model_dict}
                        # pretrained_dict_0 = {k: v for k, v in pretrained_dict_edge.items() if k in model_dict}

                            # pretrained_dict_ =pretrained_dict_

                        # pretrained_dict_ = {k: v for k, v  in zip(pretrained_dict.items(),pretrained_dict_edge.items()) if k in model_dict}
                        # pretrained_dict_0 = {k: v for k, v in pretrained_dict_edge.items() if k in model_dict}
                        # for key in pretrained_dict_edge.keys():
                        #     if key  in pretrained_dict_.keys():
                        #         print('yes')
                        ...
                        if len(pretrained_dict_) == 0:
                            pretrained_dict_ = {k.replace('module.', ''): v for k, v in model_dict_.items() if
                                                k.replace('module.', '') in model_dict}
                        if len(pretrained_dict_) == 0:
                            pretrained_dict_ = {('edge_net.model_edge.' + k): v for k, v in model_dict_.items() if
                                                'edge_net.model_edge.' + k in model_dict}
                        model_dict_ = pretrained_dict_
                        net.load_state_dict(model_dict_)
                        ...
                        for k, v in model_dict.items():
                            if k not in model_dict_  or v.size() != model_dict_[k].size():
                ...
                #         print(torch.eq(value,model_dict[key]))
                # 冻结edge层的参数
