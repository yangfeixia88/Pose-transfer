...
class PoseFlowNet(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.net_G = network.define_g(opt, structure_nc=opt.structure_nc, ngf=32,img_f=256,layers=5, norm='instance', activation='LeakyReLU',
        ...
        if self.isTrain:
            ...
            # self.Regularization = external_function.AffineRegularizationLoss(kz=5).to(opt.device)  #只有一个人flow的正则化计算
    ...
    def test(self):
        self.flow_fields, self.masks = self.net_G(self.input_P1, self.input_BP1, self.input_BP2)
        warp = self.visi(self.flow_fields[-1])

        self.save_results(warp, data_name='vis')
        # result = torch.cat([self.input_EP1, warp, self.input_EP2], 3)
