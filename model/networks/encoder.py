...
class RGBSpatioEncoder(BaseNetwork):
    ...
    def __init__(self, input_nc_structure, input_nc_rgb=3, ngf=64, z_nc=128, img_f=1024, layers=5, s_layers_down=5,
                 s_layers_up=2,
                 norm='none', activation='ReLU', use_spect=False, use_coord=False):
        super(RGBSpatioEncoder, self).__init__()
        self.encoder_rgb = RGBEncoder(input_nc_rgb, ngf, img_f, layers,
                                      norm, activation, use_spect, use_coord)
        self.encoder_str = SpatioStructureEncoder(input_nc_structure + input_nc_rgb + 1, ngf, z_nc, img_f,
                                                  s_layers_down,
...
class StructureEncoder(BaseNetwork):
    ...
    def __init__(self, input_nc, ngf=64, z_nc=128, img_f=1024, layers=6, norm='none', activation='ReLU',
        ...
        self.posterior = ResBlock(ngf * mult, 2 * z_nc, ngf * mult, norm_layer, nonlinearity, 'none', use_spect,
                                  use_coord)
...
class SpatioStructureLargeEncoder(BaseNetwork):
    ...
    def __init__(self, input_nc, ngf=64, z_nc=64, img_f=1024, layers=5, L=1, norm='none', activation='ReLU',
        ...
        for i in range(layers - 1):
            ...
            setattr(self, 'decoder' + str(i), upconv)
            setattr(self, 'jump' + str(i), jumpconv)
        self.blockup = ResBlockDecoder(ngf * mult, ngf * mult, ngf * mult, norm_layer, nonlinearity, use_spect,
                                       use_coord)

        for i in range(L):
            posterior = ResBlock(ngf * mult, ngf * mult, ngf * mult, norm_layer, nonlinearity, 'none', use_spect,
                                 use_coord)
...
class AdainStructureEncoder(BaseNetwork):
    ...
    def __init__(self, input_nc, ngf=64, z_nc=64, img_f=1024, input_size=256, layers=5, L=1, norm='none',
                 activation='ReLU',
